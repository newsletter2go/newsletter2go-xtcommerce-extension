...
const N2GO_INTEGRATION_URL = 'https://ui-staging.newsletter2go.com/integrations/connect/XTC/';
...
if (empty($this->url_data['save']) && !$this->url_data['get_singledata']) {
    ...
    if ($this->url_data['edit_id'] && $edit_data['header']['conf_XT_NEWSLETTER2GO_API_KEY_shop_' . $shop_id] && $edit_data['header']['conf_XT_NEWSLETTER2GO_API_USER_shop_' . $shop_id]) {
        ...
        if (!empty($edit_data['header']['conf_XT_NEWSLETTER2GO_API_KEY_shop_' . $shop_id]['value']) && !empty($edit_data['header']['conf_XT_NEWSLETTER2GO_API_USER_shop_' . $shop_id]['value'])) {
            ...
            $tpl_data = array('show_n2go_connect' => false);
